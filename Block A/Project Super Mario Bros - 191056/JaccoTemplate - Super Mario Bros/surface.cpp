...
void Surface::CopyFromTo(int a_srcX, int a_srcY, int a_srcPitch, Surface* a_Dst, int a_X, int a_Y)
{
	Pixel* dst = a_Dst->GetBuffer();
	Pixel* src = GetBuffer();

	int endX = a_srcX + a_srcPitch;

	int readX = a_srcX;
	int readY = a_srcY;

	int dstHeight = a_Dst->GetHeight();
	int dstWidht = a_Dst->GetWidth();
	int dstPitch = a_Dst->GetPitch();

	for (int y = 0; y < dstHeight; y++)
	{
		for (int x = 0; x < dstWidht; x++)
		{
			if (dst[x + y * dstPitch] == 0x000000)
			{
				//std::cout << "TEST";
			}

			dst[x + y * dstPitch] = src[readX + readY * a_srcPitch];
			readX++;
		}
